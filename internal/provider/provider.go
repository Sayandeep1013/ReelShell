...
// Subtitle is one subtitle track a provider can offer. Lang is a free-form
// label (e.g. "en", "English") shown nowhere yet but carried through so a
// future UI can pick a specific one; for now every subtitle a provider
// returns is loaded and mpv's own `c` (cycle sub) keybind switches between
// them (SPEC.md v2: multi-language subtitles).
...
// ResolveWithTimeout runs a single provider executable, killing it if it
